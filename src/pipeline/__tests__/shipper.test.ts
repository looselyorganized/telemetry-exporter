...
import { initLocal, getLocal, enqueue, dequeueUnshipped, enqueueArchive, dequeueUnshippedArchive, outboxDepth, archiveDepth } from "../../db/local";
import type { SupabaseClient } from "@supabase/supabase-js";
import { ProjectBlocker } from "../project-blocker";

function makeBlocker(): ProjectBlocker {
  const b = new ProjectBlocker(getLocal());
  b.loadBlocked();
  return b;
}
...
describe("Shipper.ship()", () => {
  ...
  test("dequeues pending rows and marks them shipped on success", async () => {
    ...
    const shipper = new Shipper(supabase, makeBlocker());
